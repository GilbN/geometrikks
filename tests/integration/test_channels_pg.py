...
async def test_two_writers_one_subscriber(it_asyncpg_dsn) -> None:
    """Two independent writer processes (own plugin + own backend + own
    connections) must fan in to a single subscriber -- this is the
    multi-instance shape production actually runs, not one plugin publishing
    to itself twice."""
    writer_a = ChannelsPlugin(
        backend=DegradedTolerantAsyncPgBackend(dsn=it_asyncpg_dsn),
        channels=[LIVE_EVENTS_CHANNEL],
    )
    writer_b = ChannelsPlugin(
        backend=DegradedTolerantAsyncPgBackend(dsn=it_asyncpg_dsn),
        channels=[LIVE_EVENTS_CHANNEL],
    )
    subscriber_plugin = ChannelsPlugin(
        backend=DegradedTolerantAsyncPgBackend(dsn=it_asyncpg_dsn),
        channels=[LIVE_EVENTS_CHANNEL],
    )
    async with writer_a, writer_b, subscriber_plugin:
        async with subscriber_plugin.start_subscription(LIVE_EVENTS_CHANNEL) as subscriber:
            writer_a.publish({"type": "geo_event", "data": {"hostname": "vps-1"}}, LIVE_EVENTS_CHANNEL)
            writer_b.publish({"type": "geo_event", "data": {"hostname": "vps-2"}}, LIVE_EVENTS_CHANNEL)
