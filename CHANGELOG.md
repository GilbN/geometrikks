...
- IP inspector: every IP in the app (map popup, top-IP lists, log tables, detail sheets, the decisions table) gets an inspect button that opens a side panel with what that address did in the selected range: request and error counts, bytes, timing, first and last seen, busiest hour or day, hosts and paths it hit, user agents, other locations it resolved to, the latest requests, and its CrowdSec decision. Scanner tells (high 4xx share, hosting ASN, wide path spread, bursts, malformed lines, traffic still arriving after a ban) show as chips. The panel lives at `?inspect=<ip>` so it survives navigation and can be linked. A new `GET /api/v1/analytics/ip-profile` endpoint backs it.
