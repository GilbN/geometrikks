...
| `MAP_HOME_REFRESH_HOURS` | `24` | How often this instance re-detects its own public-IP home location and refreshes its site_homes rows (hours). Separate from GEOIP_REFRESH_DAYS: the GeoLite2 database and a site's public IP change on unrelated schedules. |
